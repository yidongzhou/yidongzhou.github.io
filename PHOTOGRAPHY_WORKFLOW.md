...
1. Select roughly 8–20 photographs. Keep RAW files and full-resolution 7008 × 4672 exports outside the repository. Before preparing the album, give each JPEG a short, descriptive lowercase filename such as `ice-lake.JPG` or `keel-billed-toucan.JPG`; use hyphens instead of spaces.
...
4. Fill in the title, date, location, cover, and optional camera/lens. Write specific alt text for every photograph; captions are optional. Orientation is inferred from the dimensions. Use a landscape 3:2 photograph for the album cover because location cards use that ratio; add `layout: wide` to any image that should span both gallery columns.
   - Most albums use a single `photos:` list. For a larger set within one place, use `photo_sections:` and give each section a `title`, optional `description`, and its own `photos:` list. This keeps related series such as wildlife and hummingbirds inside the same place folder.
5. Add a short narrative below the front matter.
6. Preview locally with `bundle exec jekyll serve`. When ready, change to `published: true`, commit, and push. The Photography page lists published places alphabetically. To show an album on the homepage, add `home_featured: true` and set its position with `home_order`.
